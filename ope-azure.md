...
- **Always pass `--subscription <id>` explicitly** on every `az` command — except `az ad *` (service principals, app registrations, users, groups), which is tenant-scoped, not subscription-scoped, and errors with "unrecognized arguments" if you pass it. `az ad ds` (Domain Services) is a real subscription-scoped resource despite starting with `az ad` too — the split is by actual command group, not by prefix.
- **Verify exact release/plugin asset filenames before writing a download command** — don't guess a filename pattern from a version number. Confirmed 2026-08-06 (`operations/events/20260806_acme_challenge_migration`): a guessed win-acme plugin filename 404'd; the real name only came from querying `https://api.github.com/repos/<org>/<repo>/releases/tags/<version>` directly. Applies to any GitHub-hosted binary/plugin, not just win-acme.
- **`az network application-gateway identity assign` takes `--identity` (singular), not `--identities`, and needs the full resource ID, not just the name.** Confirmed 2026-08-06/07 (`operations/events/20260806_automatizar_clubsite_ar`) — the plural form doesn't exist and errors with "unrecognized arguments." Don't assume flag pluralization matches a sibling command (`describe-scalable-targets` does use plural `--resource-ids`, for example) — check `--help` per command rather than pattern-matching from a similar one.
- **`az vm list-ip-addresses` with a custom `--query` can silently return an empty column instead of erroring.** Confirmed 2026-08-07: querying `virtualMachine.network.privateIpAddresses` returned a table with the IP column entirely blank (not an error) — the reliable way to get a VM's private IP is `az network nic list` filtered by `virtualMachine.id`, same pattern already used for NIC discovery elsewhere in this project. Don't trust an empty/missing column as "no IP" without cross-checking via NIC list.
- **JMESPath `contains()` errors on `null`, not just empty string** — `contains(virtualMachine.id, 'X')` throws "invalid type for value: None" when some items in the list have no attached VM (orphaned NICs are common in this environment). Fix: `contains(virtualMachine.id || '', 'X')` to substitute an empty string default before calling `contains()`.
- **Application Gateway `trustedRootCertificates` only accepts a genuinely self-signed root certificate (Issuer == Subject) — intermediate CA certs are rejected with `ApplicationGatewayTrustedRootCertificateInvalidData`, regardless of DER vs. PEM/base64 encoding.** Confirmed 2026-08-07 (`operations/events/20260806_automatizar_clubsite_ar`): two Let's Encrypt intermediate certs failed with identical errors in both DER and re-encoded base64 form; only the true self-signed root (`ISRG Root X1`) uploaded successfully. Before assuming a format problem, check `openssl x509 -noout -subject -issuer` on the file — if Issuer ≠ Subject, it's an intermediate and doesn't belong in `trustedRootCertificates` at all. Intermediates must instead be served by the backend itself as part of its TLS handshake chain (confirm via the backend's local CA/Intermediate certificate store, not by trying to upload them to the gateway).
